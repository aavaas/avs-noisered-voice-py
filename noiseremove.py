...
class Noiseremover():
    def __init__(self, rate):
        ...
        self.subfeqlevelmin = self.windowlen*OVERSUBTRACTIONFREQ
        self.subfeqlevelmax = self.framelength - self.subfeqlevelmin
    ...
    def buildnoiseprofile(self, data, nosecs=0.8):
        ...
        print(statistics.mean(self.noiseprofile) ,statistics.median(self.noiseprofile))   
    
    def removenoise(self, data):
        nototalframes = np.floor(len(data) / (self.framelength))  # the total no of frames in sound
        ...
        for i in range(int(nototalframes)):
            ...
            dmag[self.subfeqlevelmin:self.subfeqlevelmax] = dmag[self.subfeqlevelmin:self.subfeqlevelmax] - (OVERSUBTRACTION * self.noiseprofile[self.subfeqlevelmin:self.subfeqlevelmax])  
    ...
    def removemusicalnoise(self, data):
        ...
        for i in range(int(nototalframes)):
            ...
            dang = np.angle(dft)
            dmag = dmag - SUBTRACTION_FACTOR * self.noiseprofile # subtract the noise spectrum by 1.5 for greater effect
            dmag[self.subfeqlevelmin:self.subfeqlevelmax] = dmag[self.subfeqlevelmin:self.subfeqlevelmax] - (OVERSUBTRACTION * self.noiseprofile[self.subfeqlevelmin:self.subfeqlevelmax])  #over subtraction    above 2kh
            dmag[dmag < 0] = 0  # because magnitude spectrum cannot be -ve
